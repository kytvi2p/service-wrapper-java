...
launchdinternal() {
    ...
    then 
        ...
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.app.parameter.1=$1 wrapper.app.parameter.2={{TestWrapperSh}} wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.name=\"$APP_NAME\" wrapper.displayname=\"$APP_LONG_NAME\" wrapper.daemonize=TRUE $ANCHORPROP $IGNOREPROP $STATUSPROP $LOCKPROP"
        eval $COMMAND_LINE
    else
...
console() {
    eval echo  'Running $APP_LONG_NAME with test action: $1...'
    getpid
    if [ "X$pid" = "X" ]
    then
        trap '' 3
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.app.parameter.1=$1 wrapper.app.parameter.2={{TestWrapperSh}} wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.name=\"$APP_NAME\" wrapper.displayname=\"$APP_LONG_NAME\" $ANCHORPROP $STATUSPROP $LOCKPROP"
        eval $COMMAND_LINE
    else
        eval echo  '$APP_LONG_NAME is already running.'
        exit 1
...
macosxstart() {
    ...
    eval echo  'Starting $APP_LONG_NAME with test action: $1.  Detected Mac OSX and installed launchd daemon.'
...
start() {
    eval echo  'Starting $APP_LONG_NAME with test action: $1...'
    getpid
    if [ "X$pid" = "X" ]
    then
        # The string passed to eval must handles spaces in paths correctly.
...
startmsg() {
    ...
    if [ "X$pid" = "X" ]
    then
        eval echo  'Starting $APP_LONG_NAME with test action: $1...  Wrapper:Stopped'
    else
        if [ "X$DETAIL_STATUS" = "X" ]
        then
            eval echo  'Starting $APP_LONG_NAME with test action: $1...  Wrapper:Running'
        else
            getstatus
            eval echo  'Starting $APP_LONG_NAME with test action: $1...  Wrapper:$STATUS, Java:$JAVASTATUS'
        fi
    fi
}
...
case "$1" in
    ...
    *)
        ...
        echo "${MSG} $0 { console <action> | start <action> | stop | restart <action> | condrestart <action> | status | install | remove | dump }"
        exit 1
        ;;
esac
